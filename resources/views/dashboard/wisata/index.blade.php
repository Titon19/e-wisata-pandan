                                    ...
                                    @foreach ($wisatas as $item)
                                        <tr>
                                            ...
                                            <td>
                                                {{ $item->wisata }} <br>
                                                <span
                                                    class="badge badge-primary">{{ ($item->category) ? $item->category->category : ''}}</span>
                                            </td>
                                            <td>{{ number_format($item->price, 2, ",", ".") }}</td>
                                            <td>{{ Str::limit($item->address, 10) }}</td>
                                            <td>
                                                <a href="{{ route('dashboard.wisata.edit', $item->id) }}"
                                                    class="btn btn-warning btn-sm">Edit</a>
                                                <form method="POST"
                                                    action="{{ route('dashboard.wisata.destroy', $item->id) }}"
                                                ...
                                                <a href="{{ route('dashboard.setup_fasilitas', $item->id) }}"
                                                    class="btn btn-primary btn-sm">Setup Fasilitas</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
