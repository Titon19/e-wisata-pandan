...
use TicketController as TicketController;
use WisataController as WisataController;
use GalleryController as GalleryController;
use TicketInController as TicketInController;
use KategoriController as KategoriController;
use ComplaintController as ComplaintController;
...
Route::post('booking', 'FrontPageController@booking')->name('booking')->middleware(['auth']);
...
Route::group(['prefix' => 'dashboard', 'as' => 'dashboard', 'middleware' => ['auth']], function () {
    ...
    Route::group(['as' => '.'], function () {
        ...
        Route::get('print/{id}', 'TransactionController@print')->name('transaction.print');
        Route::post('report/{id}', 'TransactionController@report')->name('transaction.report');
        ...
        Route::get('testimoni', 'TransactionController@testimoni_list')->name('testimoni.index');
        Route::delete('testimoni/{id}', 'TransactionController@destroyTestimoni')->name('testimoni.destroy');

        // Khusus petugas (super_admin dan staff)
        Route::group(['middleware' => ['role:super_admin,staff']], function () {
            Route::post('/dashboard/complaints', 'ComplaintController@store')->name('dashboard.complaints.store');
            ...
            Route::resource('ticket_in', TicketInController::class)->except('show');

            Route::resource('complaints', ComplaintController::class)->except(['show', 'store']);
